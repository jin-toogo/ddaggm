              ...
              try {
                ...
                const hospital = await tx.hospital.create({
                  data: {
                    ...
                    totalDoctors: data.base_총의사수
                      ? parseInt(data.base_총의사수)
                      : null,
                    latitude: data["base_좌표(Y)"]
                      ? parseFloat(data["base_좌표(Y)"])
                      : null,
                    longitude: data["base_좌표(X)"]
                      ? parseFloat(data["base_좌표(X)"])
                      : null,
                ...
                if (hasLocationDetails) {
                  await tx.hospitalLocationDetail.create({
                    data: {
                      ...
                      parkingSpaces: data.세부_주차_가능대수
                        ? parseInt(data.세부_주차_가능대수)
                        : null,
                      parkingFeeRequired: convertParkingFeeRequired(
                        data["세부_주차_비용 부담여부"]
                      ),
                ...
                if (hasOperatingHours) {
                  await tx.hospitalOperatingHour.create({
                    data: {
                      ...
                      sunStart: parseTimeString(data.세부_진료시작시간_일요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_일요일
                            )}`
                          )
                        : null,
                      sunEnd: parseTimeString(data.세부_진료종료시간_일요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_일요일
                            )}`
                        ...
                        : null,
                      monStart: parseTimeString(data.세부_진료시작시간_월요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_월요일
                            )}`
                        ...
                        : null,
                      monEnd: parseTimeString(data.세부_진료종료시간_월요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_월요일
                            )}`
                        ...
                        : null,
                      tueStart: parseTimeString(data.세부_진료시작시간_화요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_화요일
                            )}`
                        ...
                        : null,
                      tueEnd: parseTimeString(data.세부_진료종료시간_화요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_화요일
                            )}`
                        ...
                        : null,
                      wedStart: parseTimeString(data.세부_진료시작시간_수요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_수요일
                            )}`
                        ...
                        : null,
                      wedEnd: parseTimeString(data.세부_진료종료시간_수요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_수요일
                            )}`
                        ...
                        : null,
                      thuStart: parseTimeString(data.세부_진료시작시간_목요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_목요일
                            )}`
                        ...
                        : null,
                      thuEnd: parseTimeString(data.세부_진료종료시간_목요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_목요일
                            )}`
                        ...
                        : null,
                      friStart: parseTimeString(data.세부_진료시작시간_금요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_금요일
                            )}`
                        ...
                        : null,
                      friEnd: parseTimeString(data.세부_진료종료시간_금요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_금요일
                            )}`
                        ...
                        : null,
                      satStart: parseTimeString(data.세부_진료시작시간_토요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료시작시간_토요일
                            )}`
                        ...
                        : null,
                      satEnd: parseTimeString(data.세부_진료종료시간_토요일)
                        ? new Date(
                            `1970-01-01T${parseTimeString(
                              data.세부_진료종료시간_토요일
                            )}`
